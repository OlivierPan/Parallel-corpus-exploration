...
import thulac
thu1=thulac.thulac()
# This part is for chinese analyse
...
class Phrase_chi_info():
    # For chinese phrase 
    def __init__(self,phrase):
        self.longueur=len(phrase)
        self.phrase=phrase
        self.puncount=sum(phrase.count(sub) for sub in punctuationslist)

    def poschaine(self):
        pattern=re.compile(r"_.{1,2}")
        text=thu1.cut(self.phrase,text=True)
        tokenlist=text.split(" ")
        self.tokens=tokenlist
        pos_list=[]
        for token in tokenlist:
            pos=re.findall(pattern,token)
            pos_to_string="".join(pos)
            pos_list.append(pos_to_string)
        self.poslist=pos_list

        pos_list_sans_doublons=list(set(pos_list))
        posdic={}
        for pos in pos_list_sans_doublons:
            posdic[pos]=pos_list.count(pos)
        self.posdic=posdic
        

toto=Phrase_chi_info("你也许不会想到，这次的句子是什么？")
print(toto.phrase)
print(toto.puncount)
print(toto.longueur)
